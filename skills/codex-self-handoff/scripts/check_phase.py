#!/usr/bin/env python3
"""Detect the current self-handoff phase by checking signal files."""
import json
import os
import sys
...
PHASE_MAP = {
    "plan-ready.json": {
        "phase": "plan-ready",
        "actor": "architect",
        "action": "Plan ready. User must confirm before Builder mode starts.",
    },
    "build-done.json": {
        "phase": "build-done",
        "actor": "architect",
        "action": "Review: check git diff against spec in Architect mode.",
    },
    "review-fixes.json": {
        "phase": "review-fixes",
        "actor": "builder",
        "action": "Polish: fix review issues in Builder mode.",
    },
    "polish-done.json": {
        "phase": "polish-done",
        "actor": "architect",
        "action": "Re-review: verify fixes in Architect mode.",
    },
    "review-passed.json": {
        "phase": "review-passed",
        "actor": "builder",
        "action": "Commit: generate message and commit.",
    },
    "committed.json": {
        "phase": "done",
        "actor": "none",
...
Detect the current self-handoff phase by checking signal files in the handoff/ directory.
...
def detect_phase(handoff_dir=HANDOFF_DIR):
    if not os.path.isdir(handoff_dir):
        return {
            "phase": "init",
            "actor": "architect",
            "message": "No handoff directory. Start in Architect mode: write spec then plan-ready.json.",
        }
    ...
    if not found:
        return {
            "phase": "init",
            "actor": "architect",
            "message": "handoff/ exists but no signal files. Write spec then plan-ready.json.",
        }
    ...
    result = PHASE_MAP.get(
        latest, {"phase": "unknown", "actor": "unknown", "action": "Unknown state"}
    )
