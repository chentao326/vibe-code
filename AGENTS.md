...
vibe-code turns vibe coding into a calibrated experiment: assess task → blind prediction → execute → retro → evolve the rubric. It is a Codex skill collection (13 skills, 7 protocols, adapters, hooks, templates).
...
- Hook configuration in `.Codex/settings.local.json` loads at session start, not hot-reloaded.

---

# Handoff Protocol（融合后）

本项目内置两套 handoff 协议——详见 `skills/codex-claude-handoff/SKILL.md` 和 `skills/codex-self-handoff/SKILL.md`。
与 vibe-code 的融合规则见 `shared-references/handoff-vibe-bridge.md`。
